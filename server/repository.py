...
This is the ONLY layer that knows both Postgres and the engine. Route handlers
call these functions; the engine never sees a database row. SQL uses "?"
placeholders, which the db.Conn wrapper rewrites to psycopg's "%s".
"""
...
def list_exercises(conn, user_id: int) -> List[Dict[str, Any]]:
...
def search_exercises(conn, user_id: int, q: Optional[str] = None,
    ...
    if q:
        sql.append("AND name ILIKE ?")   # ILIKE = case-insensitive (Postgres)
        args.append(f"%{q}%")
    if muscle_group:
...
def get_exercise(conn, exercise_id: int) -> Optional[Dict[str, Any]]:
...
def ensure_exercise(conn, user_id: int, name: str, muscle_group=None,
                    equipment=None, movement_pattern=None) -> Dict[str, Any]:
    """
    ...
    row = conn.execute(
        "SELECT * FROM exercises WHERE (user_id IS NULL OR user_id = ?) "
        "AND lower(name) = lower(?) ORDER BY (user_id IS NULL) DESC LIMIT 1",
        (user_id, name),
    ).fetchone()
    if row:
        return row
    ...
    conn.commit()
    return conn.execute(
        "SELECT * FROM exercises WHERE user_id = ? AND lower(name) = lower(?)",
        (user_id, name),
    ).fetchone()


def exercises_with_sessions(conn, user_id: int) -> List[Dict[str, Any]]:
...
def exercise_info(row: Dict[str, Any]) -> ExerciseInfo:
    return ExerciseInfo(
...
def health_summary(conn, user_id: int, days: int = 21) -> Dict[str, Any]:
    """
    Dashboard at-a-glance data:
      * sleep: the last `days` nightly sleep readings, oldest-first (for a chart).
      * calories: {eaten, burned} for the most recent logged day. `eaten` is the
        tracked value; `burned` falls back to a body-weight estimate (~15 kcal/lb,
        ~33 kcal/kg) plus a training allowance when the day isn't measured.
    """
    rows = conn.execute(
        "SELECT date, sleep_hours, body_weight, calories FROM daily_checkins "
        "WHERE user_id=? ORDER BY date DESC LIMIT ?",
        (user_id, days),
    ).fetchall()
    rows = list(reversed(rows))  # oldest-first for charting

    sleep = [{"date": r["date"], "hours": r["sleep_hours"]}
             for r in rows if r["sleep_hours"] is not None]

    user = get_user(conn, user_id)
    unit = user["weight_unit"] if user else "lb"
    per_unit = 33.0 if unit == "kg" else 15.0

    latest = rows[-1] if rows else None
    bw = latest["body_weight"] if latest else None
    if bw is None:
        bw_row = conn.execute(
            "SELECT body_weight FROM daily_checkins WHERE user_id=? "
            "AND body_weight IS NOT NULL ORDER BY date DESC LIMIT 1",
            (user_id,),
        ).fetchone()
        bw = bw_row["body_weight"] if bw_row else (180.0 if unit == "lb" else 82.0)

    burned = round(bw * per_unit + 250)      # resting-ish + a training allowance
    eaten = latest["calories"] if (latest and latest["calories"] is not None) \
        else round(bw * per_unit)            # assume maintenance if untracked

    return {
        "sleep": sleep,
        "calories": {"eaten": int(eaten), "burned": int(burned)},
    }
...
def get_active_routine(conn, user_id: int) -> Optional[Dict[str, Any]]:
    return conn.execute(
...
def save_routine(conn, user_id: int, name: str, days: List[Dict[str, Any]],
    ...
    cur = conn.execute(
        ...
        "COALESCE(?, now()::text), COALESCE(?, now()::text)) RETURNING id",
        (user_id, name, source_template_key, changed_at, changed_at))
    routine_id = cur.fetchone()["id"]  # new row => _write_routine_days is FK-safe

    _write_routine_days(conn, user_id, routine_id, days)
    conn.execute(
        "INSERT INTO routine_history (user_id, routine_id, changed_at, "
        "change_type, snapshot_json) VALUES (?, ?, COALESCE(?, now()::text), ?, ?)",
...
def routine_context_for(conn, user_id: int,
                        exercise: Dict[str, Any]) -> Optional[RoutineContext]:
    """
